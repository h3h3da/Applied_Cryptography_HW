...
def printDDT(DDT):
    ...
    while k < len(DDT):
        temlist = []
        temlist.append(str(k+1))
        for i in DDT[k]:
            temlist.append(str(i))
        tb.add_row(temlist)
        k = k + 1
    print(tb)
...
def printLAT(LAT):
    ...
    for entry in LAT:
        ...
        if i % 256 == 0:
            temlist.append(str(int(i / 256)))
            line += str(int(i / 256)) + "\t|"
        line += str(entry) + "\t"
        # print(line)
        temlist.append(str(entry))
        # print(temlist)
        if i % 256 == 255:
            line += "\n"
...
def printAllLAT(SBoxes):
    for i in range(0, 2):
        print("LAT of ZUC " + "SBox-" + str(i))
        LATofSBOX(SBoxes[i])
        print("\n")
